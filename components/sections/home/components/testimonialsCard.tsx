...
import { TestimonialsDataProps } from '@/constant/constant'

interface Props {
  data: TestimonialsDataProps
}

const TestimonialsCard = ({ data }: Props) => {
  return (
    <div className="p-4 rounded-lg mx-4 shadow hover:shadow-lg transition-all border duration-200 w-72 shrink-0 ">
      <div className="flex gap-2">
        {data.image ? (
          ...
          <div className="size-11 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center text-white font-semibold">
            {data.name.charAt(0)}
          </div>
        )}
        <div className="flex flex-col">
          ...
          <span className="text-xs opacity-70 text-gray-600 dark:text-gray-400">{data.about}</span>
        </div>
      </div>
      {data.rating && (
      ...
      <p className="text-sm  pt-2 text-gray-700 dark:text-gray-300">{data.review}</p>
    </div>
  )
}
